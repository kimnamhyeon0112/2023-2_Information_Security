...
void send_dns_request(unsigned char *ip_req, int n_req, unsigned char *dst_ip, char *name, unsigned short transaction_id);
...
int main()
{
    ...
    while (count < 10000) {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            send_dns_response(ip_resp, n_resp, "199.43.135.53", name, i);
        }
        //##################################################################
        count++;
